...
import SalesByProductPerYear from "./SalesByProductPerYear";

export default function SalesByProduct({ code, name }) {
  ...
  const matchedLatestSales = latestSales.filter((latestSales) => {
    return (
      latestSales.customerCode === code && latestSales.customerName === name
    );
  });
  ...
  const getUniqueRecords = () => {
    ...
    uniquePartCode.map((partCode) => {
      ...
      uniqueRecord.forEach((sale) => {
        ...
        monthQty[month] += stringToNum(sale.audSalesAmount);
      });
  ...
  function updateMonthQty(filteredSales) {
    ...
    filteredSales?.forEach((sale) => {
      ...
      total[month] += stringToNum(sale.audSalesAmount);

      sum += stringToNum(sale.audSalesAmount);
    });
    ...
    <div className="max-w-5xl">
        ...
        <tbody>
          {convertNumbersInArrayToIncludeComma(uniqueRecords).map(
            (record, index) => (
              <tr key={index} className="text-md font-medium text-gray-700">
                ...
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Jul &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Jul)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Aug &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Aug)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Sep &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Sep)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Oct &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Oct)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Nov &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Nov)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Dec &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Dec)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Jan &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Jan)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Feb &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Feb)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Mar &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Mar)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Apr &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Apr)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.May &&
                    "$" + convertNumberToIncludeComma(record.monthQty.May)}
                </td>
                <td className="px-4 py-2 text-center border-b border-gray-400">
                  {record.monthQty.Jun &&
                    "$" + convertNumberToIncludeComma(record.monthQty.Jun)}
                </td>
          ...
          <tr>
            ...
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Jul") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Jul"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Aug") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Aug"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Sep") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Sep"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Oct") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Oct"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Nov") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Nov"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Dec") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Dec"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Jan") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Jan"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Feb") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Feb"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Mar") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Mar"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Apr") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Apr"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("May") &&
                "$" + convertNumberToIncludeComma(calculateTotals("May"))}
            </td>
            <td className="text-center border-b border-gray-400 bg-green-100 font-bold">
              {calculateTotals("Jun") &&
                "$" + convertNumberToIncludeComma(calculateTotals("Jun"))}
            </td>
            <td className="px-4 py-2 text-center border-b border-gray-400 bg-green-300 font-bold">
              {calculateTotal() &&
                "$" + convertNumberToIncludeComma(calculateTotal())}
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <div className="mt-5">
        <h2 className="text-2xl font-extrabold dark:text-white">
          Product sales by financial year
        </h2>
        <SalesByProductPerYear code={code} name={name} />
      </div>
    </div>
  );
}
