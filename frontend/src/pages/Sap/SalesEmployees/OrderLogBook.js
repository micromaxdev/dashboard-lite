...
export default function OrderLogBook() {
  // --------------------------Functions----------------------------
  ...
  const { salesOrderLogs, isLoading } = useSelector((state) => state.all);

  const today = new Date();
  const fiveYearsAgo = new Date(1, today.getMonth(), today.getFullYear() - 5);

  console.log("fiveYearsAgo: ", fiveYearsAgo);
  const logsWithin5Years = salesOrderLogs;

  console.log("logsWithin5Years: ", logsWithin5Years);

  const groupsBySalesEmployeeAndMonth = logsWithin5Years.reduce(
    (groups, log) => {
      const { salesEmployee, orderAmountInAUD, dateCreated } = log;
      const monthYear = moment(dateCreated, "DD/MM/YYYY").format("MM/YYYY");
      ...
      groups[key].totalOrderAmount += parseFloat(orderAmountInAUD);
      return groups;
    },
    {}
  );
  ...
  const matchedPartNotes = salesOrderLogs.filter((salesOrderLogs) => {
    return (
      stringToNum(salesOrderLogs.stockOnHand) > 0 &&
      salesOrderLogs.storageLocation
    );
  });
