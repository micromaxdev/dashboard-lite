...
export default function SalesPerformanceMonthly() {
  // --------------------------Constants----------------------------
  const {
    latestSales: ls,
    budget,
  ...
  const getTableRows = () => {
    ...
    budgetsForSelectedYear.forEach((budgetForSelectedYear) => {
      totalSalesBaseLine +=
        parseFloat(budgetForSelectedYear.salesBaseLine || 0) / 12;
      totalGpBaseLine += parseFloat(budgetForSelectedYear.gpBaseLine || 0) / 12;
      totalGpPercent += parseFloat(budgetForSelectedYear.gPPercent || 0);
      totalSalesBudget +=
        parseFloat(budgetForSelectedYear.salesBudget || 0) / 12;
      totalGpBudget += parseFloat(budgetForSelectedYear.gpBudget || 0) / 12;
    });
      ...
      <>
        {budgetsForSelectedYear.map((budgetForSelectedYear, index) => (
          <Table.Row key={index}>
            ...
            <Table.Cell>
              {formatNumber(budgetForSelectedYear.salesBaseLine / 12)}
            </Table.Cell>
            <Table.Cell>
              {formatNumber(budgetForSelectedYear.gpBaseLine / 12)}
            </Table.Cell>
            <Table.Cell>{budgetForSelectedYear.gPPercent}%</Table.Cell>
            <Table.Cell>
              {formatNumber(budgetForSelectedYear.salesBudget / 12)}
            </Table.Cell>
            <Table.Cell>
              {formatNumber(budgetForSelectedYear.gpBudget / 12)}
            </Table.Cell>
          </Table.Row>
        ))}
        {/* Total Row */}
  ...
  const getActualTableRows = () => {
    ...
    const salesByTeam = salesForSelectedYear.reduce((acc, curr) => {
      ...
      const monthNumber = getFinancialMonth(curr.invoiceDate);

      if (monthNumber > maxMonthNumber) {
        maxMonthNumber = monthNumber;
      }

      if (!acc[curr.salesEmployee].monthlySales.hasOwnProperty(monthNumber)) {
        acc[curr.salesEmployee].monthlySales[monthNumber] = 0;
        acc[curr.salesEmployee].monthlyGP[monthNumber] = 0;
      }

      acc[curr.salesEmployee].monthlySales[monthNumber] += saleValue;
      acc[curr.salesEmployee].monthlyGP[monthNumber] += gpValue;
    ...
    for (let team in salesByTeam) {
      salesByTeam[team].sales = Object.values(
        salesByTeam[team].monthlySales
      ).reduce((a, b) => a + b, 0);
      salesByTeam[team].gp = Object.values(salesByTeam[team].monthlyGP).reduce(
        (a, b) => a + b,
        0
      );
      ...
      for (let month in salesByTeam[team].monthlySales) {
        if (!totalMonthlySales[month]) {
          totalMonthlySales[month] = 0;
        }
        totalMonthlySales[month] += salesByTeam[team].monthlySales[month];
      }

      for (let month in salesByTeam[team].monthlyGP) {
        if (!totalMonthlyGP[month]) {
          totalMonthlyGP[month] = 0;
        }
        totalMonthlyGP[month] += salesByTeam[team].monthlyGP[month];
      }
    }
    ...
    const totalRow = (
      <Table.Row key="totals">
        ...
        {Object.entries(totalMonthlySales).map(([month, sales], i) => (
          <React.Fragment key={`total-month-${i}`}>
            <Table.Cell
            ...
            <Table.Cell
              ...
              key={`total-gp-${i}`}
            >
              {formatNumber(totalMonthlyGP[month])}
            </Table.Cell>
          </React.Fragment>
        ))}
      </Table.Row>
    ...
    return [
      ...Object.keys(salesByTeam).map((team, index) => {
        const monthData = [];
        for (let i = 1; i <= maxMonthNumber; i++) {
          monthData.push({
            sales: salesByTeam[team].monthlySales[i] || 0,
            gp: salesByTeam[team].monthlyGP[i] || 0,
          });
        }
        ...
        // console.log("FILTERED " + filteredBudget);

        const additionalCOGS =
          (salesTeamMemberData &&
            parseFloat(salesTeamMemberData.additionalCOGS)) ||
          0;

        const revisedGP = formatNumber(salesByTeam[team].gp - additionalCOGS);

        // console.log("sales team " + salesByTeam[team].gp);
        // console.log("Additional COGS " + additionalCOGS);
